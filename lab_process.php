...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($existing) {
            $update = $pdo->prepare("UPDATE laboratory SET laboratory_type = ?, status = ?, result = ? WHERE lab_id = ?");
            $update->execute([$lab_type, $status, $result, $existing['lab_id']]);
        } else {
            $insert = $pdo->prepare("INSERT INTO laboratory (patient_id, laboratory_type, status, result) VALUES (?, ?, ?, ?)");
            ...
            if ($paymentCheck->rowCount() > 0) {
                $pdo->prepare("UPDATE payments SET laboratory_fee = laboratory_fee + 300, total_amount = consultation_fee + (laboratory_fee + 300) WHERE patient_id = ?")->execute([$patient_id]);
            }
        }
