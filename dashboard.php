...
<!-- Appointment Modal (Weekly Calendar View) -->
...
function loadWeeklyCalendar() {
    ...
    Promise.all(days.map(day => {
        ...
        return fetch(`check_availability.php?doctor_id=${doctorId}&date=${dateStr}`)
            .then(res => res.json())
            .then(data => ({ date: dateStr, day: day, available: data.available, remaining: data.remaining, reason: data.reason || (data.available ? 'Available' : 'Full') }));
    })).then(results => {
        ...
        calendarDiv.innerHTML = '<div style="grid-column:span 7; color:red;">Error loading calendar</div>';
        console.error(err);
    });
}
