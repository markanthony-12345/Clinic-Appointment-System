...
// Cleared Today = patients who completed ALL 4 steps today
...
<div class="container">
    ...
    <main>
        ...
        <!-- Only essential quick actions remain (no duplication with nav) -->
...
<!-- Appointment Modal -->
<div id="appointment-form" class="modal"><div class="modal-content"><span class="close">&times;</span><h2>New Appointment</h2><form action="appointment_process.php" method="POST"><div class="form-row"><div class="form-group"><label>Patient ID</label><input type="number" name="patient_id" required></div><div class="form-group"><label>Doctor</label><select name="doctor_id" id="doctor_select" required onchange="checkAvailability()"><option value="">Select Doctor</option><?php $docs = $pdo->query("SELECT * FROM doctors"); while($d = $docs->fetch()): ?><option value="<?= $d['doctor_id'] ?>"><?= $d['doctor_name'] ?> (<?= $d['specialization'] ?>)</option><?php endwhile; ?></select></div></div><div class="form-row"><div class="form-group"><label>Date</label><input type="date" name="appointment_date" id="appointment_date" required onchange="checkAvailability()"></div><div class="form-group"><label>Time</label><input type="time" name="appointment_time" required></div></div><div class="form-group"><label>Lab Required?</label><select name="laboratory_required"><option>No</option><option>Yes</option></select></div><div id="availability-status" style="margin:10px 0"></div><button type="submit" class="btn primary" id="book-btn" disabled>Book Appointment</button></form></div></div>
